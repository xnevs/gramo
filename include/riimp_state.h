...
class riimp_state_mono {
 ...
 public:
  ...
  H_adjacent_vertices_type candidates() {
    struct item {
      IndexH idx;
      H_adjacent_vertices_type const & vec;
      
      item(IndexH idx, H_adjacent_vertices_type const & vec)
          : idx{idx},
            vec{vec} {
      }
    ...
    if (neighs.empty()) {
      ...
      H_adjacent_vertices_type result;
      while (true) {
        IndexH min = n;
